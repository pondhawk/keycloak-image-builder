# KDT Architecture (pointer)

The authoritative architecture lives in two places:

1. **Blueprint** — `../Keycloak_Deployment_Toolkit_Architecture_Blueprint.md`
...
A **golden model instance** is provisioned by `kcadmin` (Java 21 + Keycloak 26.x
+ toolkit), validated, sanitized by `ami-clean`, and imaged into a **per-vendor
...
- **`kcadmin`** — Bash dispatcher + `lib/` helpers + `subcommands/`.
