...
Operating rules for building KDT (blueprint §21).
...
4. **Environment-neutral AMI is sacred.** No secrets, hostnames, endpoints, or
   realm data in the image. The `ami-clean` neutrality gate must pass.
...
7. **Every milestone adds validation.** Extend `kcadmin verify` / neutrality /
...
9. **Both DB engines stay first-class.** Postgres default, MySQL co-equal in
   tests (ADR-0003).
10. **Don't test Keycloak.** Validate KDT's own work only (ADR-0012).
